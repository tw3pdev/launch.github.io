...
function Item({ p }) {
  ...
  async function init() {
    ...
    if (pContract[chain]) {
      ...
      try {
        ...
        const poolDetails = await aggrContract.methods.getPoolInfo(id).call()
        try {
          if (connected && address) {
            await checkSave()
          }
        ...
        const social = JSON.parse(strings.poolDetails)
        let auditData = { a: "", b: "" }
        if (strings.kycDetails) {
          auditData = JSON.parse(strings.kycDetails)
        }
        setString({
          logo: social.a,
          website: social.b,
        ...
        // const liq = +(bn(uintParam.amount).multipliedBy(uintParam.listMulti).div(10000).multipliedBy(UuintParam.liqRate).div(10000).div(devDecimal));
        // const tokenForPresaless = +(bn(calculate(uintParam.amount, uintParam.listMulti, UuintParam.liqRate)).div(devDecimal));
    ...
    if (connected && address) {
          let purchs = await aggrContract.methods.pinksaleGetContribution(id, address).call()
        ...
        if (bools.isWhitelist) {
          Wl = true;
          const wt = await aggrContract.methods.pinksaleGetWhitelist(id).call()
          if (connected && address) {
  ...
  async function saveUnsave() {
    if (connected) {
      const web3 = new Web3(rpcURL);
      if (await isSigned()) {
        const saves = await sendData({ req: 'save', contract: id, external: "Pinksale", chain: paramChain }, false)
          ...
          if ((string.banner).split("watch?v=")[1]) {
            return (
              <div className="rounded-2 cntr-image">
                <YouTube videoId={((string.banner).split("watch?v=")[1]).split("&ab_channel")[0]} opts={opts} />
              </div>
            );
          } else {
            return (
  ...
  async function Contribute(e) {
    ...
    try {
      ...
      if (handleSymbol(currency) != handleChain('symbol')) {
        ...
        const bbtotal = web3.utils.toWei(amount.toLocaleString(), "ether");
        const PresaleData = new web3.eth.Contract(pinksaleAbi1, id);
        await PresaleData.methods.contribute().call({ from: address, value: bbtotal }).then(async function () {
          await PresaleData.methods.contribute().send({ from: address, value: bbtotal }).then(async function () {
            toast("Success");
            loadNew();
            setAmount("");
            e.target.classList.remove("disabled");
